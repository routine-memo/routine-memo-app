...
export default function Home() {
  ...
  return (
    <main className="min-h-screen p-6 pb-24 bg-white">
      {/* 통계 카드 */}
      <section className="mb-6">
        <h2 className="text-xl font-semibold mb-4 text-black">대시보드</h2>
        <div className="grid grid-cols-2 gap-3">
          {/* 이번 주 기록 */}
          <div className="border-2 border-black p-4 bg-white rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <TrendingUp size={18} strokeWidth={2} className="text-black" />
              <p className="text-xs text-gray-600">이번 주 기록</p>
            </div>
            <p className="text-3xl font-bold text-black">{stats.thisWeekCount}<span className="text-sm ml-1">개</span></p>
          </div>

          {/* 총 기록 */}
          <div className="border-2 border-black p-4 bg-white rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <FileText size={18} strokeWidth={2} className="text-black" />
              <p className="text-xs text-gray-600">총 기록</p>
            </div>
            <p className="text-3xl font-bold text-black">{stats.totalEntries}<span className="text-sm ml-1">개</span></p>
          </div>
        </div>
      </section>
      ...
      {/* 해빗 트래커 */}
      <section className="mb-6">
        <h2 className="text-lg font-semibold mb-3 text-black">기록 현황</h2>
      ...
      {stats.albumsWithNotification.length > 0 && (
        <section className="mb-6">
          <h2 className="text-lg font-semibold mb-3 text-black flex items-center gap-2">
      ...
      <section>
        <div className="flex items-center justify-between mb-3 pb-2 border-b border-gray-200">
          ...
          <Link href="/records" className="text-sm text-gray-600 font-medium flex items-center gap-1 hover:underline">
            <span>전체 보기</span>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            <div className="w-20 h-20 mx-auto mb-4 rounded-full border-2 border-gray-200 bg-gray-50 flex items-center justify-center">
              <Calendar size={36} strokeWidth={1.5} className="text-gray-400" />
            </div>
