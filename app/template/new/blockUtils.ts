...
export const redistributeRow = (
  ...
  console.log(`🔄 redistributeRow - 행 ${row}:`, {
    ...
    totalSpan,
    blocks: occupyingBlocks.map(b => ({ id: b.id, row: b.row, colSpan: b.colSpan }))
  });
  ...
  // 남은 공간을 블록들에게 균등 분배
  const blocksToExpand = occupyingBlocks.length;
  ...
  const redistributedBlocks = occupyingBlocks.map((block, index) => {
    const extraSpan = colsPerBlock + (index < remainder ? 1 : 0);
    const newBlock = {
      ...block,
      row: block.row, // row는 유지 (멀티행 블록이 원래 row 유지)
  ...
  console.log(`✅ 재분배 완료:`, redistributedBlocks.map(b => ({ id: b.id, colStart: b.colStart, colSpan: b.colSpan })));
